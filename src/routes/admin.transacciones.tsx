...
import { Loader2, CheckCircle2, XCircle, Undo2, Banknote, Smartphone, CreditCard } from "lucide-react";
...
type Tx = {
  id: string; order_id: string; payment_method: string; amount: number;
...
const methodIcon: Record<string, typeof Banknote> = {
  efectivo: Banknote, yape: Smartphone, plin: Smartphone, tarjeta: CreditCard,
};
...
function TxPage() {
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("payment_transactions")
        .select("id, order_id, payment_method, amount, status, reference, notes, confirmed_at, created_at, order:orders(number, doc_kind, customer:customers(full_name))")
  ...
  const filtered = useMemo(() => data.filter(t => {
    ...
    return String(t.order?.number ?? "").includes(s)
  ...
  const setStatus = async (tx: Tx, status: "confirmado" | "rechazado" | "reembolsado", extra?: { reference?: string; notes?: string }) => {
    ...
    try {
      ...
      setConfirmTx(null); setReference(""); setNotes("");
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">Confirma o rechaza pagos por Yape, Plin, tarjeta o efectivo</p>
        </div>
        <Input placeholder="Buscar N°, cliente o referencia..." value={q} onChange={(e) => setQ(e.target.value)} className="w-72" />
      </div>
          ...
          {isLoading ? <div className="py-16 grid place-items-center"><Loader2 className="size-8 animate-spin text-gold" /></div> : (
            <Table>
              <TableHeader><TableRow>
                <TableHead>Fecha</TableHead><TableHead>Pedido</TableHead><TableHead>Cliente</TableHead>
              ...
              <TableBody>
                {filtered.map(t => {
                  ...
                  return (
                    <TableRow key={t.id}>
                      <TableCell className="text-xs whitespace-nowrap">{new Date(t.created_at).toLocaleString("es-PE")}</TableCell>
                      <TableCell className="font-mono">{t.order ? String(t.order.number).padStart(6, "0") : "—"}</TableCell>
                      <TableCell>{t.order?.customer?.full_name ?? "—"}</TableCell>
                      <TableCell><div className="flex items-center gap-1.5 capitalize text-sm"><Icon className="size-4 text-gold" />{t.payment_method}</div></TableCell>
                      <TableCell className="text-xs text-muted-foreground">{t.reference ?? "—"}</TableCell>
                      <TableCell className="text-right font-semibold text-gold">S/ {Number(t.amount).toFixed(2)}</TableCell>
                      <TableCell><Badge className={`${statusColor[t.status]} border-0 capitalize`}>{t.status}</Badge></TableCell>
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          {t.status === "pendiente" && (
                            <>
                              <Button size="sm" variant="ghost" className="text-green-500 hover:text-green-500 hover:bg-green-500/10"
                                onClick={() => { setConfirmTx(t); setReference(t.reference ?? ""); setNotes(t.notes ?? ""); }}>
                                <CheckCircle2 className="size-4 mr-1" />Confirmar
                              </Button>
                              <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive hover:bg-destructive/10"
                                disabled={busy} onClick={() => setStatus(t, "rechazado")}>
                                <XCircle className="size-4 mr-1" />Rechazar
                              </Button>
                            </>
                          )}
                          {t.status === "confirmado" && (
                            <Button size="sm" variant="ghost" className="text-blue-500 hover:text-blue-500 hover:bg-blue-500/10"
                              disabled={busy} onClick={() => setStatus(t, "reembolsado")}>
                              <Undo2 className="size-4 mr-1" />Reembolsar
                            </Button>
                          )}
                        </div>
                      </TableCell>
                ...
                {filtered.length === 0 && <TableRow><TableCell colSpan={8} className="text-center py-10 text-muted-foreground">Sin transacciones</TableCell></TableRow>}
              </TableBody>
            </Table>
          )}
        </CardContent>
      ...
      <Dialog open={!!confirmTx} onOpenChange={(o) => !o && setConfirmTx(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Confirmar pago — S/ {Number(confirmTx?.amount ?? 0).toFixed(2)}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
              <Label>Referencia / N° de operación</Label>
              <Input value={reference} onChange={(e) => setReference(e.target.value)}
                placeholder={confirmTx?.payment_method === "tarjeta" ? "Últimos 4 dígitos" : "Código de operación"} />
            </div>
            <div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmTx(null)}>Cancelar</Button>
            <Button className="bg-gradient-gold text-primary-foreground" disabled={busy}
